 ...
 */
  constructor(pageContext, opts) {
    ...
    this.isShowResult = opts.isShowResult
    // this.canvasId = opts.canvasId
    this.r = opts.r || 4
    this.endCallBack = opts.callback
    this.tapStart = opts.tapStart
    this.localImg = opts.localImg
    this.lastX = 0
    this.lastY = 0
    this.minX = ''
    this.minY = ''
  ...
  init() {
    const { canvasWidth, canvasHeight, imageResource, localImg, maskColor } = this
    const self = this
    ...
    } else if (localImg && localImg != ''){
      self.ctx.drawImage(localImg, 0, 0, canvasWidth, canvasHeight)
      self.ctx.draw()
    } else {
    ...
    // this.page.setData({
    //   ['scratch.awardTxt']: this.page.data.awardTxt,
    // })
  }
