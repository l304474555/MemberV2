...
Page({
  ...
  changeCardInfo: function (cardId,req) {
    var list =[];
    if (req=="gift")
    {
      list = this.data.MaterialList;
    }else
    {
      list = this.data.integralList;
    }
    var obj;
    for (var i = 0; i < list.length; i++) {
      obj = list[i];
      if (obj.Id == cardId) {
        obj.ReStocks = obj.ReStocks-1;
        if(obj.ReStocks==0){
          obj.CardStatus = '已抢光';
        }
        break;
  ...
  ExchangeCoupe: function (event) {
    ...
    //库存为0 
    var itemRestocks = event.target.dataset.restocks
    if (itemRestocks==0){
      wx.showToast({
        title: '该券已抢光',
      })
    ...
    let content = '兑换此券需抵扣' + that.data.needJifen + '活动分\n\t请确认是否抵扣？';
    if (coupontype == 3)//礼品
    {
      if (carStatus != "立即领取") {
        return;
      }
      content = '兑换此礼品需抵扣' + that.data.needJifen + '活动分\n\t请确认是否抵扣？';
    } else {
      if (carStatus != "立即领取") {
        return;
      }
  ...
  yesExcange: function (e) {
    ...
    myjCommon.getLoginUser(function (user) {
      ...
      if (that.data.coupontype == 3) //礼品
      {
        ...
        myjCommon.callApi({
          interfaceCode: "WxMiniProgram.Service.BasketballGCChangeGift",
          biz: {
          ...
          success: function (res) {
            ...
            } else if (res.Code == "3001") //金币不够
            {

              that.setData({
                noteContent: '您的活动分余额不足，暂不能兑换此礼品!',
                isEnave: true,
                isChange: false
              });
            ...
            } else if (res.Code == "0") //兑换成功
            {
              ...
              //库存-1;
              that.changeCardInfo(carid,"gift");
        ...
        myjCommon.callApi({
          interfaceCode: "WxMiniProgram.Service.BasketballGCChangeMPCoupon",
          biz: {
            sessionId: user.sessionId,
            cardId: carid,
            cCnt: needJifen,
          ...
          success: function (res) {
            ...
            {
              //库存-1;
              that.changeCardInfo(carid, "coupon");
              that.setData({
                isGetSucess: true,
                isChange: false
              });
    ...
    myjCommon.getLoginUser(function (user) {
      ...
      myjCommon.callApi({
        ...
        biz: { 
          typeId: 0, 
          couponType: 1, 
          pageSize: that.data.gifpageSize,
          pageIndex: that.data.gifpageIndex, 
          sessionId: user.sessionId, 
          channel: 26, 
          cityName: app.currCity
          },
  ...
  changeCity: function () {
    ...
    wx.navigateTo({
      url: '/pages/yhq_dw/yhq_dw?target=basketball'
    });
  },
})