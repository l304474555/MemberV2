...
Page({
  ...
  onShow: function () {
    ...
    if ((storeinfo != '' && this.data.isupdateshop)) {
      this.setData({
        currentType: -1,
        pageIndex: 1,
        cardList: [],
  ...
  GetGiftBagActivity: function (areaname) {
    ...
    myjCommon.getLoginUser(function (user) {
      ...
      //新人送积分 先注释
      //that.GivingNewMemberGC(areaname, user.sessionId);
    });
  },
        ...
        if (storeinfo != '' && storeinfo.storeCode != '' && storeinfo.storelat != '' && storeinfo.storelng != '') {
          ...
          wx.getLocation({
            ...
            success: function (res) {
              ...
              /**经度 */
              var longitude = res.longitude;
              if (that.data.currStoreInfo.storeCode != '') {
                that.GetCouponIndexData(that.data.currStoreInfo.storeCode, latitude, longitude);
              } else {
                that.GetCouponIndexData("", latitude, longitude);
              }
